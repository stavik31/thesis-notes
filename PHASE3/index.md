...
| Page | Date | Summary |
|------|------|---------|
| [[wiki/progress/2026-07-08]] | 2026-07-08 | **★ Stage 3 rewrite COMPLETE + live.** GAT fully replaced by per-type cluster+share+XGBoost (8 sections), run nationally → production `risk_scores.csv` (19.8M rows). **Decision:** objective → `reg:logistic` (bounded [0,1] share; verified metric-identical to squared-error on Manchester holdout). New `make_plots.py` (5 training plots; GAT plots archived). Feature importance is type-specific (car→counts, cycle/hgv→share). Routing expected to diverge into ~3 groups (car/two-wheeler/freight) where hotspots sit on the path |
