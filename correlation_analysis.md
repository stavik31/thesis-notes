...
Run from the thesis root. Outputs go to `analysis/`.
...
# ── Load ──────────────────────────────────────────────────────────────────────
print("Loading...")
collision = pd.read_csv(DATA_DIR / 'dft-road-casualty-statistics-collision-last-5-years.csv', low_memory=False)
vehicle   = pd.read_csv(DATA_DIR / 'dft-road-casualty-statistics-vehicle-last-5-years.csv',   low_memory=False)
casualty  = pd.read_csv(DATA_DIR / 'dft-road-casualty-statistics-casualty-last-5-years.csv',  low_memory=False)
...
| File | Contents |
|------|----------|
| `analysis/correlation_results.csv` | All features ranked by effect size with p-values |
| `analysis/multicol_categorical.csv` | Cramér's V matrix for top 15 categorical features |
| `analysis/multicol_numeric.csv` | Spearman matrix for top 15 numeric features |
| `analysis/correlation_analysis.png` | Bar chart of top 20 + severity distribution |

## How to run

```bash
cd /home/vik-esoc/Desktop/thesis
# copy the code block above into a .py file and run it, or paste into a notebook
python3 correlation_analysis.py
```

## Reading the results

- **Effect size > 0.3** — meaningful association, include in retrieval index
- **Effect size 0.1–0.3** — weak but real, include with lower weight
- **Effect size < 0.1** — near-zero, flag for removal
- **Multicollinearity V > 0.7** — redundant pair, keep the one with higher effect size vs severity
- **Spearman ρ sign** — positive means higher value → worse severity (Fatal); negative means the inverse
